...
**规则：**
1. `description` — 补充性描述，与域已有描述互补。如果没有新的补充角度，留空字符串
2. `sub_problems` — 从本项目发现的、域已有子问题列表中没有的新子问题。没有则留空数组
3. `best_practices` — 从本项目提炼的、域已有最佳实践中没有的新实践。没有则留空数组
4. 每条 ≤40 字，具体到该项目的做法，不要泛泛而谈
```
